...
!!! note "Özet"
    ...
    Ana çıkarım basit: yüksek enflasyonlu bir pazarda nominal gelir tek başına yeterli değildir. Gelir; reel gelir, hacim ve fiyat etkisinden arındırılmış KPI'larla birlikte değerlendirilmelidir.
...
Bu etkileri ayırmak için nominal geliri, enflasyona göre düzeltilmiş reel geliri, müşteri/sipariş/adet hacmini ve gerçekleşen ürün fiyatı değişimlerini karşılaştırdım.
...
-   :lucide-trending-up:{ .lg .middle } __Nominal gelir yükseldi__
    ...
    Toplam nominal gelir analiz döneminde yaklaşık **13.11 milyar TRY** seviyesine ulaştı.

-   :lucide-chart-no-axes-column-decreasing:{ .lg .middle } __Reel gelir zayıfladı__

    ---

    TÜFE düzeltmesinden sonra toplam reel gelir yaklaşık **7.87 milyar TRY** oldu; bu daha zayıf bir satın alma gücü performansına işaret ediyor.
...
-   :lucide-tags:{ .lg .middle } __Fiyatlar keskin hareket etti__
    ...
    Ürün seviyesinde ödenen fiyatlar belirgin şekilde arttı; birkaç ana kategoride fiyat artışları **%100** seviyesinin üzerine çıktı.
...
Analiz dönemi boyunca toplam nominal gelir yaklaşık 13.11 milyar TRY oldu. Buna karşılık enflasyon düzeltmesinden sonra toplam reel gelir yaklaşık 7.87 milyar TRY seviyesinde kaldı.
...
### TÜFE baskısı yorumu değiştirecek kadar güçlüydü
...
Bu enflasyonist ortam, nominal gelirin performansı değerlendirmek için neden tek başına yeterli olmadığını açıklar. İşletme yalnızca cari fiyatlı satışları izlerse, fiyat seviyesi artışını gerçek iş büyümesi sanabilir.

### Siparişler ve adetler hacim büyümesini desteklemedi
...
- Müşteri sayısı yalnızca sınırlı değişti.
- Satılan adet ve sipariş sayısı yaklaşık %3.6 geriledi.

Bu, nominal gelir artışının esas olarak daha fazla müşteriden, daha fazla siparişten veya daha fazla satılan adetten kaynaklanmadığını gösterir. İşin hacim tarafı genel olarak stabil ya da biraz daha zayıftı.

### Ürün fiyatları enflasyon açıklamasını doğruladı

Ürün seviyesindeki analiz enflasyon hipotezini destekler. Binlerce ürün genelinde gerçekleşen ödenen fiyatlar dönem boyunca ciddi biçimde arttı.

Dashboard, ana ürün kategorilerinde ortalama ödenen fiyat artışlarının %100'ün üzerine çıktığını gösterir. Örneğin sıcak içecekler yaklaşık %121 ile en yüksek artışlardan birini gösterirken temizlik, meyve-sebze, bebek ürünleri ve süt ürünleri gibi kategorilerde de güçlü fiyat büyümesi görülür.

Bu, zayıf hacim büyümesine rağmen nominal gelirin neden arttığını açıklar: işletme ürünleri daha yüksek nominal fiyatlardan satıyordu; fakat bu yüksek fiyatlar daha güçlü reel gelire dönüşmedi.

## İş Etkileri

!!! tip "Yönetim çıkarımı"

    3A Superstore'un gelir büyümesi büyük ölçüde hacim kaynaklı değil, enflasyon kaynaklıydı.

    Başka bir ifadeyle market zaman içinde daha fazla TRY tahsil etti; fakat reel satın alma gücü açısından daha az kazandı.

    Yönetim yalnızca nominal geliri izlerse işletme gerçekte olduğundan daha sağlıklı görünebilir. Yüksek enflasyonlu bir pazarda reel gelir, reel ortalama sipariş değeri, hacim ve fiyat etkisinden arındırılmış KPI'lar iş sağlığına daha güvenilir bir bakış sağlar.
...
Reel geliri korumak için işletmenin enflasyonun üzerinde büyüme üreten stratejilere ihtiyacı olur:
...
- müşteri elde tutmayı ve satın alma sıklığını iyileştirmek,
...
- daha zayıf bölgeleri ve şubeleri bölgesel dashboardlar üzerinden incelemek.
